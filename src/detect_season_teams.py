...
def _prev_season_teams() -> set[str]:
    """지난 시즌 팀 집합 — 실제 최종 순위(standings) 기준. 실패 시 teammeta."""
    try:
        import datastore as ds
        st = ds.read_table("standings", league="EPL")
...
def main(argv: list[str] | None = None) -> int:
    ...
    title = args.season_title or tracking_season_title()
    label = _label_from_title(title)
    print(f"[season-teams] tracking: {title} ({label})")
