"""
Ligue 1 감독 프로필 수집기 — fetch_bundes_managers 와 동일 패턴.

위키 "2026-27 Ligue 1"(최신) 우선 + "2025-26 Ligue 1"(현재 표시 18팀) 폴백 병합.
캐논 18팀만 남기고(노이즈·승격전용팀 제외) manager_profiles_Ligue1 생성.

사용: python src/fetch_ligue_managers.py
"""
...
SOURCE_NEXT = tracking_season_title().replace("Premier League", "Ligue 1")
SOURCE_CUR = "2025-26 Ligue 1"

OUR = ["Angers", "Auxerre", "Brest", "Le Havre", "Lens", "Lille", "Lorient", "Lyon",
       "Marseille", "Metz", "Monaco", "Nantes", "Nice", "PSG", "Paris FC", "Rennes",
       "Strasbourg", "Toulouse"]
ALIAS = {"Paris Saint-Germain": "PSG", "Paris Saint-Germain F.C.": "PSG",
         "Olympique de Marseille": "Marseille", "Olympique Marseille": "Marseille",
         "AS Monaco": "Monaco", "AS Monaco FC": "Monaco", "Olympique Lyonnais": "Lyon",
         "Lille OSC": "Lille", "LOSC Lille": "Lille", "OGC Nice": "Nice",
         "RC Lens": "Lens", "Racing Club de Lens": "Lens",
         "Stade Rennais": "Rennes", "Stade Rennais F.C.": "Rennes", "Stade Rennais FC": "Rennes",
         "RC Strasbourg": "Strasbourg", "RC Strasbourg Alsace": "Strasbourg",
         "FC Nantes": "Nantes", "Toulouse FC": "Toulouse",
         "Stade Brestois 29": "Brest", "Stade Brestois": "Brest", "AJ Auxerre": "Auxerre",
         "Angers SCO": "Angers", "Le Havre AC": "Le Havre", "FC Metz": "Metz",
         "FC Lorient": "Lorient", "Paris FC": "Paris FC"}
...
def _mapped(title: str) -> dict:
    try:
        ...
        print(f"[ligue-mgr] {title} 파싱 실패: {exc}", file=sys.stderr)
        return {}
...
def main() -> int:
    ...
    if not cur and not nxt:
        print("[ligue-mgr] 위키 감독표 파싱 실패", file=sys.stderr)
        return 1
    ...
    print(f"[ligue-mgr] 감독 {len(profiles)}팀 — 사진·전술 보강 중…")
    ...
    out = data_path("manager_profiles", "Ligue1", ext="json")
