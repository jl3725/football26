...
def team_ratings(team: str, traits: pd.DataFrame, standings,
    ...
    if unit_metrics is not None and team in unit_metrics.index:
        ...
        if standings is not None and (standings["squad"] == team).any():
            ...
            points_idx = _power_from_pct(_rank_pct(st["points"], team, high_is_good=True), 58, 95)
            gd_idx = _power_from_pct(_rank_pct(st["gd"], team, high_is_good=True), 58, 95)
        ...
        # 순수 마무리(슈팅 위협)만 — xA·빅찬스(창출)는 미드필드로 귀속해 공격 이중계산 방지
        ...
        # 공격 = 공격수 기량 + 마무리 효율(npxG/슈팅) 중심. 팀 총득점(result) 의존을 낮추고
        # 창출(creation)은 미드필드로 이관 → 시스템/중원으로 득점하는 팀의 공격 과대평가 방지.
        attack = _blend_scores([
            (attack_result, 0.42),       # 실제 득점 결과(주 동력)
            (shot_power, 0.28),          # 마무리 효율(npxG/유효슈팅)
            (attack_quality_idx, 0.16),  # 공격수 시장가치는 보조 prior로만
            (set_piece_power, 0.08),
            (pressure_power, 0.06),
        ]) or iv("attack_index")
        # 미드 = 창출/전진 비중↑(공격에서 이관). 점유만 좋고 성적 안 나오는 팀은 폼으로 보정.
        midfield = _blend_scores([
            ...
            (form_idx, 0.08),   # 중원 장악이 실제 성과로 이어졌는지 결과 보정
        ...
        # 포지션군 레벨 보정 — 팀 득점은 시스템(중원 창출)에 크게 의존하므로 공격 지수는 소폭↓,
        # 창출 기여가 실제로 일어나는 미드필드는 소폭↑ (전 팀 공통, 순위 영향 없는 레벨 이동).
...
def overview_scout_dossier_html(team: str, standings: pd.DataFrame | None,
    ...
    form = int(rating_map.get("시즌 폼", overall))
    ...
    # OVR 링 티어 색 — analytics OVR 배지와 동일 체계(골드/에메랄드/블루/슬레이트)
    if overall >= 85:
        ring_light, ring_deep, tier_name = "#f9dd7e", "#c2902a", "ELITE"
    elif overall >= 80:
        ring_light, ring_deep, tier_name = "#5fe2a2", "#0a9c62", "STRONG"
    elif overall >= 75:
        ring_light, ring_deep, tier_name = "#6fb1fb", "#1f63cf", "SOLID"
    else:
        ring_light, ring_deep, tier_name = "#9fb0c3", "#536276", "DEVELOPING"
    _ring_pct = max(2, min(99, overall))

    def index_bar(label: str, value: int) -> str:
        c = rating_color(value)
        # PES식 세그먼트 게이지 — 20칸 중 value/5 만큼 점등(부분칸은 반투명)
        segs_n = 20
        filled = max(0.0, min(segs_n, value / (100 / segs_n)))
        cells = []
        for i in range(segs_n):
            if i < int(filled):
                cell = f"background:{c};box-shadow:0 0 5px {c}99"
            elif i < filled:
                cell = f"background:{c};opacity:.45"
            else:
                cell = "background:rgba(255,255,255,.14)"
            cells.append(f"<div style='flex:1;height:9px;border-radius:2px;{cell}'></div>")
        return f"""
        <div style="margin-bottom:11px">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:5px">
            <span style="font-size:11px;color:rgba(255,255,255,.70);font-weight:800">{label}</span>
            <span class="pes-num" style="font-size:17px;color:{c};text-shadow:0 1px 4px {c}77">{value}</span>
          </div>
          <div style="display:flex;gap:2px">{''.join(cells)}</div>
        </div>"""
    ...
    def tile(label: str, value: str, sub: str = "") -> str:
        ...
        <div style="background:rgba(255,255,255,.09);border:1px solid rgba(255,255,255,.14);
                    ...
                    text-align:center">
          <div class="pes-num" style="font-size:26px;color:#fff;line-height:1">{value}</div>
    ...
    return f"""
    <style>
    @import url('https://fonts.googleapis.com/css2?family=Teko:wght@500;600;700&family=Oswald:wght@500;600;700&display=swap');
    .pes-num{{font-family:'Teko',sans-serif!important;font-style:italic;font-weight:700;letter-spacing:.5px}}
    .pes-cap{{font-family:'Oswald',sans-serif!important;font-weight:600;text-transform:uppercase}}
    </style>
      ...
      <div style="display:grid;grid-template-columns:1.02fr 1.08fr .92fr;gap:0;position:relative;min-height:300px">
        <div style="padding:20px 20px 18px;border-right:1px solid rgba(255,255,255,.12);
          ...
          <div style="margin:18px auto 0;width:170px;height:170px;border-radius:50%;display:flex;
                      ...
                      conic-gradient({ring_light} 0,{ring_deep} {_ring_pct}%,rgba(255,255,255,.16) {_ring_pct}% 100%);
                      box-shadow:0 0 24px {ring_deep}66,0 14px 34px rgba(0,0,0,.22)">
            <div style="width:134px;height:134px;border-radius:50%;background:linear-gradient(145deg,#111827,#05070b);
                        display:flex;align-items:center;justify-content:center;text-align:center;border:1px solid {ring_light}55">
              <div>
                <div class="pes-cap" style="font-size:11px;color:{ring_light};letter-spacing:2px">{tier_name}</div>
                <div class="pes-num" style="font-size:60px;line-height:.82;color:#fff;margin-top:3px;
                            text-shadow:0 2px 12px {ring_deep}aa">{overall}</div>
                <div class="pes-cap" style="font-size:10px;color:rgba(255,255,255,.62);margin-top:5px;letter-spacing:1px">SQUAD OVR · FORM {form}</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div style="padding:22px 22px 18px">
          <div style="display:flex;align-items:center;justify-content:space-between;gap:14px;margin-bottom:15px">
            <div>
              <div class="pes-cap" style="font-size:11px;color:rgba(255,255,255,.62);letter-spacing:1.8px">MATCH INTEL</div>
              <div style="font-size:22px;font-weight:950;color:#fff;line-height:1.05;margin-top:3px">핵심 경기 신호</div>
            </div>
            <div class="pes-cap" style="padding:7px 11px;border-radius:999px;background:rgba(255,255,255,.13);
                        border:1px solid rgba(255,255,255,.16);font-size:12px;letter-spacing:.8px">
              GD <span class="pes-num" style="font-style:italic">{gd_str}</span>
            </div>
          </div>
          ...
          <div style="margin-top:12px;padding:12px 14px;border-radius:13px;background:rgba(255,255,255,.09);
            ...
            <div class="pes-cap" style="font-size:11px;color:rgba(255,255,255,.62);letter-spacing:1.4px">DISCIPLINE</div>
            <div class="pes-num" style="font-size:22px;color:#fff;margin-top:4px">{discipline}</div>
        ...
        <div style="padding:22px 20px 18px;background:rgba(0,0,0,.22);border-left:1px solid rgba(255,255,255,.12)">
          <div class="pes-cap" style="font-size:11px;color:rgba(255,255,255,.64);letter-spacing:1.6px;
          ...
          <div style="margin-top:14px;padding:12px;border-radius:12px;background:rgba(255,255,255,.08);
            ...
            <div class="pes-cap" style="font-size:11px;color:rgba(255,255,255,.6);letter-spacing:1.4px">SCOUT VERDICT</div>
