...
// 경량 force 레이아웃(의존성 없음). ~15노드 × 200iter → useMemo 에서 즉시.
function layout(nodes: SGNode[], edges: { a: string; b: string; matches: number }[]) {
  const N = nodes.length;
  const idx: Record<string, number> = {};
  nodes.forEach((n, i) => (idx[n.id] = i));
  const pos = nodes.map((_, i) => ({
    x: W / 2 + W * 0.32 * Math.cos((2 * Math.PI * i) / N),
    y: H / 2 + H * 0.32 * Math.sin((2 * Math.PI * i) / N),
  }));
  ...
  for (let it = 0; it < 220; it++) {
    ...
    for (let i = 0; i < N; i++) for (let j = i + 1; j < N; j++) {
      ...
      const d2 = dx * dx + dy * dy || 1, d = Math.sqrt(d2), f = 2800 / d2, ux = dx / d, uy = dy / d;
    ...
    for (const e of edges) {
      ...
      const L = 55 + (1 - e.matches / maxM) * 95;   // 강할수록 가깝게
      const f = (d - L) * 0.03, ux = dx / d, uy = dy / d;
      fx[i] += ux * f; fy[i] += uy * f; fx[j] -= ux * f; fy[j] -= uy * f;
    }
    for (let i = 0; i < N; i++) {
      fx[i] += (W / 2 - pos[i].x) * 0.02; fy[i] += (H / 2 - pos[i].y) * 0.02;
